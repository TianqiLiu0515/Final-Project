...
def constraint_colorChange(img):
    width = img.shape[0]
    height = img.shape[1]
...
# reconmmand size is 10
def constraint_myFisheye(img, size):
    rows = img.shape[0]
    cols = img.shape[1]
    ...
    r = size  # recommand 10 for 100 x 100 picture
...
def constraint_myBlur(img, strength):
    rows = img.shape[0]
    cols = img.shape[1]
    cv2.blur(img, (strength, strength), img)
    return img

def constraint_circle(Copy, radius, x, dis, newgrad):
    i = 100
    k = 3
    for i in range(0, int(x / 2), 50):
        # print(i, dis + newgrad * i)
        cv2.circle(Copy, (i, int(dis + newgrad * i)), k, (255, 255, 255), radius)
        # cv2.circle(Copy, (0, 0), k, (255, 255, 255), 30)
        cv2.circle(Copy, (x - i, int(dis + newgrad * i)), k, (255, 255, 255), radius)
    return Copy
